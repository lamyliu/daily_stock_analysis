...
### 6. 四维度价值评估（检查项6）
对每只股票进行四维度评分（每项0-25分，总分100分），结果写入检查项6：
- **ROE持续性**（0-25分）：ROE>15%且稳定=20+分，ROE<8%或下降趋势=<10分
- **债务安全性**（0-25分）：负债权益比<1且流动比率>1.5=20+分，负债权益比>2=<10分
- **自由现金流**（0-25分）：FCF为正且>净利润50%=20+分，FCF为负=<5分
- **PE估值合理**（0-25分）：PE<行业均值=20+分，PE>50或无盈利=<10分
- 总分80+：✅估值优秀；60-79：⚠️估值合理；<60：❌估值偏高或有风险
- 价值陷阱警告：PE低但ROE持续下降、高股息但FCF为负、行业萎缩
